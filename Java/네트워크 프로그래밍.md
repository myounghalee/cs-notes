...
관련: [[Java]] · [[동시성]] · [[가상 스레드]] · [[Netty]] · [[../Infrastructure/RESTful API|RESTful API]]

## 목차
- [[#네트워크 프로그래밍이란? — 컴퓨터 경계를 넘는 프로그래밍]]
- [[#소켓이란? — 통신의 양쪽 끝]]
...
평소 짜는 프로그램은 대부분 **한 컴퓨터 안에서 끝나요** — 변수를 계산하고, 파일을 읽고, 화면에 출력하고. **네트워크 프로그래밍(Network Programming)** 은 이 범위를 넘어서, **서로 다른 프로세스(보통 다른 컴퓨터에 있는)가 네트워크를 통해 데이터를 주고받도록 만드는 프로그래밍**이에요.

```mermaid
graph LR
    subgraph "일반 프로그래밍 — 한 컴퓨터 안에서 끝남"
        Local["내 프로그램"] --> File["파일/메모리"]
    end
    subgraph "네트워크 프로그래밍 — 컴퓨터 경계를 넘음"
        A["내 컴퓨터의 프로그램"] <-->|"네트워크"| B["다른 컴퓨터의 프로그램"]
    end
```

**왜 특별히 더 까다로울까요?**

- 상대방이 같은 프로세스가 아니라서, 함수 호출하듯 데이터를 바로 주고받을 수 없어요 — 바이트로 직렬화해서 **네트워크 프로토콜(TCP/UDP)** 규칙에 맞게 보내야 해요.
- 네트워크는 언제든 끊기거나 느려질 수 있고, 상대방이 지금 응답 가능한 상태인지 미리 알 수 없어요.
- 서버는 보통 **동시에 여러 상대와 통신**해야 해서, [[동시성]]에서 다룬 문제까지 함께 따라와요.

그래서 언어/OS는 이런 통신을 위해 표준화된 창구인 **소켓(Socket)** 을 제공하고, Java는 이 위에 `Socket`/`ServerSocket`, NIO, `HttpClient` 같은 도구들을 쌓아뒀어요. 이 문서는 그 도구들을 순서대로 살펴봐요.
...
> 실무에서는 이런 저수준 NIO API를 직접 다루기보다, 이걸 훨씬 편하게 감싼 **Netty** 같은 프레임워크를 써요. [[../Java/Redis 클라이언트|Redis 클라이언트]]에서 다룬 Lettuce도 내부적으로 Netty를 기반으로 만들어졌어요. Netty의 EventLoop, ChannelPipeline 구조는 [[Netty]]에서 자세히 다뤄요.
