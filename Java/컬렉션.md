...
같은 `Map` 인터페이스를 구현하지만, **내부 자료구조 자체가 완전히 달라요** — 그래서 성능 특성과 할 수 있는 일이 크게 갈려요.

```mermaid
graph TD
    subgraph "HashMap — 버킷 배열"
        B0["버킷 0"]
        B1["버킷 1 → apple"]
        B2["버킷 2"]
        B3["버킷 3 → banana → cherry"]
    end
    subgraph "TreeMap — 레드-블랙 트리"
        Root["banana"] --> L["apple"]
        Root --> R["cherry"]
    end
```

- **HashMap**: key의 `hashCode()`를 계산해서 **버킷 배열의 어느 칸에 넣을지** 정해요. 순서는 이 해시값에 따라 결정되기 때문에 **사람이 예측할 수 있는 순서가 아니에요.**
- **TreeMap**: key를 **레드-블랙 트리**(자가 균형 이진 탐색 트리)에 저장해요. 트리 구조 자체가 "왼쪽 자식은 더 작은 값, 오른쪽 자식은 더 큰 값"이라는 규칙을 유지하기 때문에, **순회하면 항상 key 순서대로 정렬**돼서 나와요.
...
**정렬 기준을 커스텀하고 싶다면** [[컬렉션]]에서 다룬 `Comparator`를 생성자에 넘기면 돼요.

```java
Map<String, Integer> desc = new TreeMap<>(Comparator.reverseOrder()); // 내림차순으로 정렬
```

**null 처리도 달라요.** `HashMap`은 key로 `null`을 **하나까지 허용**하지만(`null`의 해시값을 그냥 0으로 취급), `TreeMap`은 key를 저장할 때마다 **다른 key와 크기를 비교해야** 하는데 `null`은 비교할 방법이 없어서 **`NullPointerException`을 던져요.**

```java
Map<String, Integer> hashMap = new HashMap<>();
hashMap.put(null, 1); // ✅ 정상 동작

Map<String, Integer> treeMap = new TreeMap<>();
treeMap.put(null, 1); // ❌ NullPointerException! (비교 불가능)
```

**TreeMap만의 특별한 메서드**도 있어요 — 정렬된 구조라서 가능한, "범위 조회" 기능이에요.

```java
TreeMap<Integer, String> map = new TreeMap<>();
map.put(10, "a"); map.put(20, "b"); map.put(30, "c");

map.firstKey();        // 10 — 가장 작은 key
map.lastKey();         // 30 — 가장 큰 key
map.floorKey(25);      // 20 — 25 이하인 것 중 가장 큰 key
map.ceilingKey(25);    // 30 — 25 이상인 것 중 가장 작은 key
map.headMap(20);       // {10=a} — 20보다 작은 것들만
map.tailMap(20);       // {20=b, 30=c} — 20 이상인 것들만
```

| 구분                     | HashMap                           | TreeMap              |
| ---------------------- | --------------------------------- | -------------------- |
| 내부 구조                  | 해시 버킷 배열                          | 레드-블랙 트리             |
| 순서                     | 없음(해시값에 따라 결정)                    | 항상 key 기준 정렬         |
| get/put 시간복잡도          | 평균 O(1)                           | O(log n)             |
| null key               | 1개 허용                             | 허용 안 함(NPE)          |
| 범위 검색(floor/ceiling 등) | ❌                                 | ✅                    |
| 언제 쓰나                  | 순서 상관없이 빠른 조회가 필요할 때 (대부분의 기본 선택) | 정렬된 순회, 범위 검색이 필요할 때 |

> **한 줄 정리**: "그냥 빠르게 저장하고 찾기만 하면 된다"면 `HashMap`, **"항상 정렬된 순서가 필요하거나, 범위로 조회해야 한다"** 면 `TreeMap`이에요.
...
| HashMap vs TreeMap의 핵심 차이는? | 내부 구조(버킷 배열 vs 레드-블랙 트리), 속도(O(1) vs O(log n)), null key 허용 여부, 범위 검색 가능 여부 |
...
### Q. HashMap과 TreeMap의 내부 구조 차이가 어떤 결과로 이어지나?
A. HashMap은 해시 버킷 배열에 저장해 순서가 없는 대신 평균 O(1)로 빠르고, TreeMap은 레드-블랙 트리에 저장해 항상 key 순으로 정렬되지만 조회에 O(log n)이 걸린다. 이 구조 차이 때문에 TreeMap만 firstKey/floorKey 같은 범위 검색 메서드를 제공할 수 있다.

### Q. TreeMap에 null을 key로 넣으면 왜 예외가 발생하나?
A. TreeMap은 새 key를 저장할 때마다 기존 key들과 크기를 비교해 트리에서의 위치를 정해야 하는데, null은 다른 값과 비교할 방법이 없어 NullPointerException이 발생한다. 반면 HashMap은 비교 없이 해시값만으로 위치를 정하기 때문에 null key를 하나까지 허용한다.
