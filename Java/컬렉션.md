...
`Queue`(FIFO), `Deque`(양쪽 다 삽입/삭제 가능), 그리고 우선순위로 꺼내는 `PriorityQueue`는 자료구조 자체의 원리(LIFO/FIFO, 힙 구조)가 핵심이라 별도 문서에서 깊이 다뤘어요.

- **Queue/Deque/Stack**: [[../Data Structures/스택과 큐|스택과 큐]]
- **PriorityQueue**: [[../Data Structures/힙과 우선순위 큐|힙과 우선순위 큐]]
